  ...
  const handleCheckout = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 401) {
        ...
        router.push(
          `/login?redirect=${encodeURIComponent("/cart?checkout=true")}`
        );
        return;
      }
      ...
      setErrorMessage(
        "We could not process your order. Please check your connection and try again."
      );
      setIsProcessingCheckout(false);
    }
  }, [
    ...
    <section className="space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-sm text-neutral-600">
          Review your items, adjust quantities, and proceed to checkout when you
          are ready.
        </p>
      </header>
            ...
            {items.map((item) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 rounded-full border border-neutral-200 px-2 py-1">
                  ...
                  <span className="min-w-[2ch] text-center text-sm font-semibold text-neutral-900">
                    {item.quantity}
                  </span>
