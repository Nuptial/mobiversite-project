...
try {
  const out = execSync('git ls-files "apps/web/**" | egrep "\\.tsx?$" || true', { stdio: ['ignore','pipe','pipe'] }).toString().trim()
  if (out) {
  ...
  const out = ex2('find apps/web -type f \\( -name "*.ts" -o -name "*.tsx" \\) | cat', { stdio: ['ignore','pipe','pipe'] }).toString().trim()
  if (out) {
